...
exports.testNews = async (req, res) => {
  newsModel.aggregate(
    [
      {
        $lookup: {
          from: "users",    //another table name
          localField: "email",  //another table field name to fetch
          foreignField: "user_id", // model table fields to match with
          as: "userdetails", // feild name to send data in 
        },
      },
    ],
    function (err, result) {
